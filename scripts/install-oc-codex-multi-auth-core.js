...
function printHelp() {
	console.log(`Usage: ${PACKAGE_NAME} [command] [options]\n\n` +
		...
		"  --full             Install compact base models plus 70 explicit selector entries\n" +
		"  --legacy           Force explicit legacy config (70 preset model entries)\n" +
...
export async function runInstaller(argv = process.argv.slice(2), options = {}) {
	...
	if (!pluginOnly && configMode === "legacy") {
		log("Note: Legacy config writes 70 explicit preset entries and is also safe for older OpenCode versions.");
	}
